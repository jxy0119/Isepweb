...
%>
	...
	<%if(aL1.size()==0){ %><p>Not found any comments waitting for confirming !</p><br>
	...
	%>
		...
		<tr class="active">
		    ...
		    <td><%=Cb.getState() %></td>
		    <td><%=Cb.getDate() %></td>
		    <td><a href="<%=request.getContextPath()%>/OperateServlet?f=5&id=<%=Cb.getId() %>">accept</a></td><td><a href="<%=request.getContextPath()%>/OperateServlet?f=6&id=<%=Cb.getId() %>">refuse</a></td>
		</tr>
		<%} }%>
		<p><a href="<%=request.getContextPath()%>/OperateServlet?f=8">see the results of the comments</a></p>
	    <%
	}%>
	</table>	
